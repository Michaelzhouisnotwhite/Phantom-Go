...
class Board(object):
    ...
    def copy(self, b_cpy: Optional["Board"]=None):
        """
        实现对象的拷贝功能
        :param b_cpy: Board对象
        :return:
    ...
    def illegal_remove(self, v):
        ...
        """
        self.switch_side()
        self.play_my_side(v, guess=True, show=False)
        self.switch_side()

        self.showboard()

    def illegal_play(self, v):
        """棋子处于被提状态，无法落下，调用则个函数
        ...
        b_cpy = Board()
        self.copy(b_cpy)
        b_cpy.play_my_side(v, show=False)
        sg_index = b_cpy.id[v]

        self.switch_side()

        fail_list = []
        for pos in b_cpy.sg[sg_index].libs:
            return_flag = self.play_my_side(pos, show=False, guess=True)
            if return_flag != 0:
                fail_list.append(pos)

        for i in fail_list:
            self.play_my_side(i, show=False, guess=True)

        self.switch_side()
        self.showboard()

    def take_remove(self, v=PASS, vlist=None):
        """
        若棋子因为提子移走，调用这个函数
        :param v:
        """
        self.switch_side()
        if v != PASS and vlist == None:
            flag = []
            for d in dir4:
                value = self.play_my_side(v + d, show=False, guess=True)
                if value != 0:
                    flag.append((v + d))
                # else:
                #     self.move_cnt -= 1
        ...
        if v == PASS and vlist is not None:
            flag = []
            for vv in vlist:
                for d in dir4:
                    if (vv + d) not in vlist:
                        i = self.play_my_side(vv + d, show=False, guess=True)
                        if i != 0:
                            flag.append((vv + d))
                    # else:
                    #     self.move_cnt -= 1
    ...
    def legal(self, v):
        ...
        stone_cnt = [0, 0]  # [白棋眼， 黑棋眼]
        atr_cnt = [0, 0]
        for d in dir4:  # [1, EBSIZE, -1, -EBSIZE] [1, 11, -1, -11]
            nv = v + d
            c = self.color[nv]
        ...
        return (atr_cnt[int(self.turn == 0)] != 0 or atr_cnt[self.turn] < stone_cnt[self.turn])

    def eyeshape(self, v, pl):
        """
        当前v位置能否形成眼 or 是否会填眼?
        :param v: 位置
        :param pl: self.turn,当前player的颜色；int(pl == 0) 判断是否为pl相对颜色:如果传1黑,那么就是0白.
        :return: 是否能构成:bool
        """
        # 如果pass那么一定不能构成眼
        if v == PASS:
    ...
    def play_my_side(self, v, not_fill_eye=True, guess: bool=False, show: bool=True):
        """
        走子
        Args:
            v ([type]): 位置
            not_fill_eye (bool, optional): 是否不填眼. Defaults to True.
            guess (bool, optional): Defaults to False.
            show (bool, optional): Defaults to True.

        Returns:
            int: 1 = illegal; 0 = legal
        """
        # 异常操作
        # 不合法返回1
        # 不想要填眼但是填了眼 返回2
        ...
        # 如果能走，且满足眼的要求
            ...
            if v == PASS:
                ...
                if self.remove_cnt == 1 and \
                        self.sg[sg_id].lib_cnt == 1 and \
                        self.sg[sg_id].size == 1:
                    # 劫的位置定义为，被叫吃的位置
                    self.ko = self.sg[sg_id].v_atr
    ...
    def feature(self):
        ...
        for i in range(KEEP_PREV_CNT):
            # self.prev_color: 两个(121, )的列表，记录棋盘
    ...
    def hash(self):
        ...
        :return: hash_code
        """
        return (hash(self.color.tostring()) ^ hash(self.prev_color[0].tostring()) ^ self.turn)
...
if __name__ == '__main__':
    ...
    b = Board()
    b.turn = 0
    b.play_my_side(90, show=False)
    b.play_my_side(91, show=False)
    b.play_my_side(92, show=False)
    b.play_my_side(93, show=True)
    # b.turn = 1
    # b.play_my_side(101, show=False)
    # b.play_my_side(102, show=False)
    # b.play_my_side(103, show=False)
    # b.play_my_side(105, show=False)
    # b.play_my_side(94, show=False)
    # b.play_my_side(82, show=False)
    # b.play_my_side(83, show=False)
    # b.play_my_side(81, show=False)
    # b.play_my_side(80, show=False)
    # b.play_my_side(79, show=False)
    # b.play_my_side(89)
    """
       A  B  C  D  E  F  G  H  J
    9 .  X  X  X  .  X  .  .  .  9
    ...
    """
    b.turn = 0
    return_flag = b.play_my_side(104)
    print()